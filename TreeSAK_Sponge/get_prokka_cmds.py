...
selected_genome_txt = '/Users/songweizhi/Desktop/gnm124.txt'
gnm_dir             = '/srv/scratch/z5265700/Shan_z5095298/z5095298/Weizhi/Shen/bac_c_ts_gnm_folder'
gnm_ext             = 'fna'
prokka_cmds_txt     = '/Users/songweizhi/Desktop/gnm124_prokka.txt'
...
for each_gnm in open(selected_genome_txt):
    gnm_id = each_gnm.strip().split('.fna')[0]
    pwd_gnm_file = '%s/%s.%s' % (gnm_dir, gnm_id, gnm_ext)
    prokka_cmd     = 'prokka --force --compliant --cpus 1 --kingdom Bacteria --prefix %s --locustag %s --strain %s --outdir %s_prokka_wd %s' % (gnm_id, gnm_id, gnm_id, gnm_id, pwd_gnm_file)
    prokka_cmds_txt_handle.write(prokka_cmd + '\n')
prokka_cmds_txt_handle.close()