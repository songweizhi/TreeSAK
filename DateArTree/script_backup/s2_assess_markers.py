...
def assess_markers(trimmed_aln_dir, gnm_meta_txt, present_pct_cutoff_list, assess_summary_1_txt, assess_summary_2_txt):
...
gnm_group_txt                   = '%s/gnm_domain.txt'                       % wd
deltall_stdout_txt              = '%s/nohup.out'                            % wd
...
# outputs
assess_summary_deltall_txt      = '%s/assessment_deltall.txt'               % wd
assess_summary_1_txt_by_marker  = '%s/assessment_pa_1.txt'                  % wd
assess_summary_2_txt_by_marker  = '%s/assessment_pa_2.txt'                  % wd
assess_summary_txt_by_genome    = '%s/assessment_pa_by_genome.txt'          % wd
...
# assess markers
cutoff_to_qualified_marker_dict, gnm_to_marker_dict, marker_to_gnm_dict = assess_markers(trimmed_aln_dir, gnm_group_txt, present_pct_cutoff_list, assess_summary_1_txt_by_marker, assess_summary_2_txt_by_marker)
...
for each_cutoff in cutoff_to_qualified_marker_dict:
    ...
    pwd_op_txt = '%s/assessment_pa_qualified_marker_%s.txt' % (wd, each_cutoff)
...
for each_marker_set in marker_set_dict:
    ...
    if len(current_marker_set) >= min_marker_num:
        ...
        pwd_iqtree_dir    = '%s/%s_iqtree_wd'       % (wd, each_marker_set)
        ...
        # get guide tree
        get_guide_cmd = '/home-user/software/iqtree/iqtree2.1.3 -s %s_concatenated.phy --prefix %s_guide_tree --seqtype AA -m LG -T 12 -B 1000 --alrt 1000' % (each_marker_set, each_marker_set)
        # print(get_guide_cmd)

        # run C60 + PMSF
        c60_pmsf_cmd = '/home-user/software/iqtree/iqtree2.1.3 -s %s_concatenated.phy --prefix %s --seqtype AA -m LG+G+F+C60 -T 12 -B 1000 --alrt 1000 -ft %s_guide_tree.treefile' % (each_marker_set, each_marker_set, each_marker_set)
        # print(c60_pmsf_cmd)

        # generate job script
        pwd_js = '%s/js_%s_iqtree.sh' % (wd, each_marker_set)
